...
sampleClusters::sampleClusters(Real x, Real y, Real z) : SampleBase(x, y, z)
{
  sh = 0;
  cl = 0;
  cn = 0;
...
MaterialBase *
sampleClusters::lookupMaterial(Point & pos)
{
...
// dr == 0.0 means looking if we are inside the cluster
int
sampleClusters::lookupCluster(Point & pos, Real dr)
{
  Real dif[3], r2;
...
void
sampleClusters::initSpatialhash(int x, int y, int z)
{
  kn[0] = x;
  kn[1] = y;
  kn[2] = z;
...
void
sampleClusters::clearSpatialHash()
{
...
void
sampleClusters::reallocClusters(int n)
{
  if (n > cnm)
  {
...
void
sampleClusters::addCluster(Real x, Real y, Real z, Real r)
{
  int k[3], l;
  // Real cb[4];
...
void
sampleClusters::addRandomClusters(unsigned int n, Real r, Real dr, SimconfType * simconf)
{
  Point npos;
  ...
  for (unsigned int i = 0; i < n; ++i)
  {
    ...
    if (i + 1 % 10000 == 0)
      fprintf(stderr, " %d (%d) clusters added\n", i + 1, cn);
  }
}
