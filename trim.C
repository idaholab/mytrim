...
// correct for maximum available range in current _material by increasing maximum impact parameter
    ...
    if (range < _ls)
    {
      ...
      std::cout << "CC " << _pka->_pos(0) + _pka->_dir(0) * range << ' ' << _pka->_pos(1) +
      _pka->_dir(1) * range << '\n'; std::cout << "CC " << '\n';*/
...
// correct for maximum available range in current _material by dropping recoils randomly (faster)
    ...
    if (_den > _pka->_E)
      _den = _pka->_E; // avoid negative energy
    _pka->_E -= _den;
...
{
  if (_pka->_state == IonBase::MOVING || _pka->_state == IonBase::LOST)
    return;
